def create_customer(first_name, last_name, email, password)
  ...
  BEGIN
    ...
    INSERT INTO customer (firstname, lastname, email, password) VALUES (i_firstname, i_lastname, i_email, i_password);
    ...
    COST 100;
    GRANT EXECUTE ON FUNCTION create_customer(text, text, text, text) TO lorenzo;

    SELECT * FROM create_customer('#{first_name}', '#{last_name}', '#{email}', '#{password}');
  """
end

def create_address(customer, address_line1, address_line2 = 'NULL', postcode, country, region, city)
  """
  ...
  LANGUAGE 'plpgsql' VOLATILE SECURITY DEFINER
  COST 100;
  GRANT EXECUTE ON FUNCTION save_address(bigint, text, text, text, text, text, text)TO lorenzo;

  SELECT * FROM save_address('#{customer}', '#{address_line1}', '#{address_line2}', '#{postcode}', '#{country}', '#{region}', '#{city}');
  """
end

def create_order(customer, address)
  """
  ...
  LANGUAGE 'plpgsql' VOLATILE SECURITY DEFINER
  COST 100;
  GRANT EXECUTE ON FUNCTION create_order(integer,integer) TO lorenzo;
  SELECT * FROM create_order('#{customer}', '#{address}');
  """
end

def deliver_order(order_id, changed_by)
  """
  ...
  LANGUAGE 'plpgsql' VOLATILE SECURITY DEFINER
  COST 100;
  GRANT EXECUTE ON FUNCTION deliver_order(integer,text) TO lorenzo;
  SELECT * FROM deliver_order('#{order_id}', '#{changed_by}');
  """
end
